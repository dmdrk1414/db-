...
public class CollegeTableView extends JPanel {
    ...
    private static final Integer JSCROLL_PANE_HIGHT = 70; // add 2. 스크롤 펜 높이 조정
    private final String[] tableColumnNames;
    private final DefaultTableModel mainTable;
    private final JTable table;

    public CollegeTableView() {
    ...
    public void creatTable() {
        String[] columRow;
        List<College> list = new DatabaseManagement().findAllCollege(); // add 4. 원하는 객체 선택

        for (College college : list) { // add 5. 원하는 객체 선택
            columRow = college.ofArray();
