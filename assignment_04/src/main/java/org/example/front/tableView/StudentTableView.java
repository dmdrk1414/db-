...
public class StudentTableView extends JPanel {
    ...
    private static final String TABLE_NAME = TableStringConstant.TABLE_NAME_STUDENT.getName(); // add 1. 원하는 테이블 선택
...
//        table.getColumnModel().getColumn(1).setPreferredWidth(220); // add 3. 원하는 컬럼의 넓이
    ...
    public void creatTable() {
        String[] columRow;
        List<Student> list = new DatabaseManagement().findAllStudent(); // add 4. 원하는 객체 선택

        for (Student student : list) { // add 5. 원하는 객체 선택
            columRow = student.ofArray();
