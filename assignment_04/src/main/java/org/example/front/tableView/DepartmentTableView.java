...
public class DepartmentTableView extends JPanel {

    private static final String TABLE_NAME = TableStringConstant.TABLE_NAME_DEPARTMENT.getName(); // add 1. 원하는 테이블 선택
...
//        table.getColumnModel().getColumn(1).setPreferredWidth(220); // add 3. 원하는 컬럼의 넓이
    ...
    public void creatTable() {
        String[] columRow;
        List<Department> list = new DatabaseManagement().findALlDepartment(); // add 4. 원하는 객체 선택

        for (Department department : list) { // add 5. 원하는 객체 선택
            columRow = department.ofArray();
