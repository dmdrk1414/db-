...
public class InstructRankTableView extends JPanel {

    private static final String TABLE_NAME = TableStringConstant.TABLE_NAME_INSTRUCTOR_RANK.getName(); // add 1. 원하는 테이블 선택
    ...
    public InstructRankTableView() {
        ...
        table.getColumnModel().getColumn(1).setPreferredWidth(120); // add 3. 원하는 컬럼의 넓이
    ...
    public void creatTable() {
        String[] columRow;
        List<InstructorRank> list = new DatabaseManagement().findAllInstructorRank(); // add 4. 원하는 객체 선택

        for (InstructorRank instructorRank : list) { // add 5. 원하는 객체 선택
            columRow = instructorRank.ofArray();
